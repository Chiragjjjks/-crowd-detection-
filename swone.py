...
# Configuration
input_image_path = 'C:/Users/Neha KB/Desktop/humanhead/res_crowd/c7.jpg'  # Path to the single image
output_folder = 'C:/Users/Neha KB/Desktop/humanhead/output/new'
csv_file_path = os.path.join(output_folder, 'counts.csv')
window_size = 320  # Smaller window size for detecting smaller objects
stride = 160     # Smaller stride for more overlap and coverage
confidence_threshold = 0.5  # Lower threshold to detect more objects
nms_threshold = 0.4 # Balance between reducing duplicates and keeping detections
...
def draw_sliding_window_boxes(image, window_size, stride):
    (h, w) = image.shape[:2]
    ...
    for y in range(0, h - window_size + 1, stride):
        for x in range(0, w - window_size + 1, stride):
            ...
            cv2.rectangle(image_with_windows, top_left, bottom_right, (0, 255, 0), 2)
...
if os.path.exists(input_image_path):
    ...
    # Draw sliding window boxes on the image
    image_with_windows = draw_sliding_window_boxes(image, window_size, stride)
    window_output_path = os.path.join(output_folder, 'sliding_window_boxes.jpg')
    cv2.imwrite(window_output_path, image_with_windows)
    ...
    # Save results
    output_path = os.path.join(output_folder, f"{os.path.splitext(os.path.basename(input_image_path))[0]}_detected.jpg")
    ...
    with open(csv_file_path, 'a', newline='') as csv_file:
        csv_writer = csv.writer(csv_file)
        csv_writer.writerow([os.path.basename(input_image_path), head_count, person_count])

    # Print counts for debugging
    print(f"Processed {os.path.basename(input_image_path)} - Heads: {head_count}, Persons: {person_count}")
